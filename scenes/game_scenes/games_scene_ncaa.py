...
class NCAAGamesScene(GamesScene):
    ...
    def display_scene(self):
        ...
        if display_yesterday:
            if (hasattr(self, 'data_previous_day') and self.data_previous_day['saved_date'] != dates_to_display[0]) or not hasattr(self, 'data_previous_day'):
                self.data_previous_day = {
                    ...
                    'games': data.ncaa_data.get_games(dates_to_display[0], self) # Get data for previous date.
                }
        ...
        self.data = {
            ...
            'games': data.ncaa_data.get_games(dates_to_display[-1], self), # Get data for current day. Current day will always be the last element of dates_to_display.
        }
        ...
        if self.data['games_previous_pull']: # Only applicable if there's a previous copy to compare to.
            for game in self.data['games']:
                if game['status'] not in ['PRE']: # Not applicable if the game hasn't started yet.
                    # Match games between data pulls.
                    matched_game = next(filter(lambda x: x['game_id'] == game['game_id'], self.data['games_previous_pull']))

                    if matched_game['status'] not in ['PRE']: # Not applicable if the game hasn't started yet in the previous pull.
